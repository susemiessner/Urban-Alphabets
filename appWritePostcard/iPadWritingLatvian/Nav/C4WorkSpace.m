...
-(void)setup {
    //load the defaults
    self.userName=@"CCNRiga";
    self.currentLanguage=@"Latvian";
...
-(void)loadAlphabetFromServer{
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL    URLWithString:@"http://www.ualphabets.com/requests/Riga/alphabet.php"]];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    for (int i=0; i<[self.latvian count]; i++) {
        ...
        for (int j=0; j<[self.receivedLettersArray count]; j++) {
            ...
            if ([self.latvian[i] isEqualToString: letter.letterName]) {
    ...
    //NSLog(@"length latvian: %i, length new alphabet: %i", [self.latvian count], [self.theNewAlphabetArray count]);
    for (int i=0; i<[self.latvian count]; i++) {
        for (int j=0; j<[self.theNewAlphabetArray count]; j++) {
            Letter *newLetter=self.theNewAlphabetArray[j];
            //NSLog(@"i:%i, j: %i latvian letter: %@   new alphabetletter: %@,   finalAlphabetArrayLength: %i",i,j, self.latvian[i], newLetter.letterName, [self.finalAlphabetArray count]);
            if ([self.latvian[i] isEqualToString: newLetter.letterName]) {
                //first time add all
