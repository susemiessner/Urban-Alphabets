...
-(void)setup {
    //load the defaults
    self.currentLanguage= @"English/Portugese";
    self.userName=@"Typemotion Fact Liverpool";
...
-(void)loadCurrentQuestion{
    ...
    NSString *urlString=@"http://www.ualphabets.com/requests/connected/question_iPadEnglish.php";
...
-(void)loadAlphabetFromServer{
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL    URLWithString:@"http://www.ualphabets.com/requests/Liverpool/alphabet.php"]];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    for (int i=0; i<[self.english count]; i++) {
        ...
        for (int j=0; j<[self.receivedLettersArray count]; j++) {
            ...
            if ([self.english[i] isEqualToString: letter.letterName]) {
    ...
    for (int i=0; i<[self.german count]; i++) {
        for (int j=0; j<[self.theNewAlphabetArray count]; j++) {
            ...
            if ([self.german[i] isEqualToString: newLetter.letterName]) {
                ...
                //if it's not in the array add the letter from the English array
               ...
               /* newLetter.identifier = 0;
                ...
                NSLog(@"added missing : %@", newLetter.letterName);*/
                 ...
                 letterToAdd.letterName= self.english[i];
...
-(void)displayPostcard{
    ...
    if (![newCharacter  isEqual:@""] && [self.postcardArray count] >0) { //if something was added
...
-(void)addLetterToPostcard{
    NSLog(@"new character: %@, %@", newCharacter, self.postcardText);
    //letters
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    newCharacter=text;
    self.postcardText= [textView.text stringByAppendingString:newCharacter];
