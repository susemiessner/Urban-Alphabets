...
void ofApp::updateAlphabet(){
    ...
    if (currentURL==currentAlphabet && currImgNoAlphabet[5]>40 && allAlphabet[currImgNoAlphabet[5]]._xPos<-354) {
        counterDrawAlphabet=0;
        blendInfo=0;
        //now go to that screen
        currentURL=URLsToLoad[currentURLNo];
...
void ofApp::drawPostcards(){
    ...
    //draw title
    postcardsTitle.draw(ofGetWidth()-postcardsTitle.width-15, 61);
    
    //draw question
    questions[currentQuestionNumber].draw(5,61);
