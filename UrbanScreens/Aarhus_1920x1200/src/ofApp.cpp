...
void ofApp::loadURL_recentPostcards(ofHttpResponse &response){
    ...
    if (response.status==200 && response.request.name=="async_req") {
        ...
        if (switchedFromVideo==true) {
            currentURL=URLsToLoad[currentURLNo];
            printf("%i",currentURLNo);
            switchedFromVideo=false;
        }
    }
}
...
void ofApp::updatePostcards(){
    ...
    if (currentURL==recentPostcards && counterNumberPostcards>lengthPostcards*FRAME_RATE*4 && noOfPostcards>3) {
        ...
        currImgNo=0;
        if (counterNumberPostcards==lengthPostcards*FRAME_RATE-1) {
            postcardBlendedIn=1;
        }
...
void ofApp::updateAlphabet(){
    ...
    if (currentURL==currentAlphabet && currImgNoAlphabet[5]>40 && allAlphabet[currImgNoAlphabet[5]]._xPos<-310) {
        counterDrawAlphabet=0;
        blendInfo=0;
        //now go to that screen
        currentURL=URLsToLoad[currentURLNo];
...
void ofApp::drawPostcards(){
    ...
    //title and question
    //blend in
    if(counterPostcardsAndLetters<FRAME_RATE && currImgNo==0 && postcardBlendedIn==0){
        //blendInfo+=8;
        ofSetColor(255, 255, 255, blendInfo);
    }
    //blend out
    else if(counterPostcardsAndLetters>=FRAME_RATE*(lengthPostcards-1) && currImgNo==allPostcards.size()-1){
        //blendInfo-=8;
        ofSetColor(255, 255, 255, blendInfo);

    } else if (postcardBlendedOut==0){
        ofSetColor(255);
    }

    //draw title
    postcardsTitle.draw(ofGetWidth()-postcardsTitle.width-15, 20);
    
    //draw question
    questions[currentQuestionNumber].draw(5,20);
...
void ofApp::drawLetters(){
    ...
    lettersTitle.draw(ofGetWidth()-lettersTitle.width-5, 0);
...
void ofApp::drawAlphabet(){
    ...
    alphabetTitle.draw(ofGetWidth()-alphabetTitle.width-5, 20);
    ofDisableAlphaBlending();
}
